...
const { API_KEY, SYMBOL_IBM, DATA_TYPE_JSON, OUT_PUT_SIZE_COMPACT, CONTENT_TYPE_JSON, DYNAMIC_INPUT } = require("../support/utils/constants");
...
describe('[GET] TIME SERIES DAILY', { tags: '@smoke' }, () => {
    ...
    /**
     * [Test-1]: Verify Successful retrieval of Daily Time Series Data - Json format, compact
     ...
     */
    it.only('[Test-1.1]: Verify Successful retrieval of Daily Time Series Data - json format, compact', () => {
        get_time_series_daily(API_KEY, SYMBOL_IBM, DATA_TYPE_JSON, OUT_PUT_SIZE_COMPACT)
            .then(response => {
                ...
                expect(Object.keys(response.body['Time Series (Daily)'])).to.have.length(100)

                // Verify a few randomly selected data points with another source (Eg. GoogleFinance), the datapoint values should me equal
            });
    });
